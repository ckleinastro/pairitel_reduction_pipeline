...
class ImageContainer:
    ...
    def _get_job_status(self,timeout=120.0):
    ...
    def _replace_new_wcs(self,delnew=True):
        ...
        for c in tmp2.ascardlist():
            if c.key != "COMMENT":
                tmp[0].header.update(c.key,str(c.value),c.comment)
...
class AstrometrySolver:
    ...
    job_dict = {"uname": "Christopher Klein","email": "cklein@astro.berkeley.edu", "fsunit" :"arcsecperpix",\
        "fstype-ul": 1, "fsu": 2.2, "fsl": 0.85, "xysrc": "img", "parity": 0, "index": "10arcmin", "tweak": 1,\
    ...
    def _make_request(self,imgfile=None,pixel_size_range = [0.8,1.2], tweak_astrometry=True):
    ...
    def __str__(self):
        ...
        a += "*"*100 + "\n"
        self.return_status = results[0][2]
        for r in results:
